...
import com.viyu.apuzzletrans.activity.PuzzleActivity;
import com.viyu.apuzzletrans.util.Puzzle;

@SuppressLint("ViewConstructor")
public class PuzzleView extends View {
	public static enum ShowNumbers {
		NONE, SOME, ALL
	};
	...
	private ShowNumbers showNumbers = ShowNumbers.NONE;
	...
	private PuzzleActivity activity = null;
	...
	public PuzzleView(Context context, Puzzle puzzle) {
		...
		textPaint = new Paint();
		textPaint.setColor(getResources().getColor(R.color.metal_silver));
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tiles.length; i++) {
			...
			if (di == tiles[di]) {
				...
				matchRight = tiles[di] + 1 < tiles.length - 1 && (di + 1) % puzzleWidth > 0 && (tiles[di] + 1) % puzzleWidth > 0 && (di + 1) < tiles.length && (di + 1) % puzzleWidth > 0
						&& tiles[di + 1] == tiles[di] + 1;
			...
			if (!solved && (showNumbers == ShowNumbers.ALL || (showNumbers == ShowNumbers.SOME && di != tiles[di]))) {
				canvas.drawText(String.valueOf(originalTiles[i] + 1), (targetRect.left + targetRect.right) / 2, (targetRect.top + targetRect.bottom) / 2 - (textPaint.descent() + textPaint.ascent())
						/ 2, textPaint);
			}
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			boolean flag = startDrag(event);
			Log.e("PuzzleView", "startDrag:" + flag);
			return flag;
		} else if (event.getAction() == MotionEvent.ACTION_MOVE) {
			boolean flag = updateDrag(event);
			Log.e("PuzzleView", "updateDrag:" + flag);
			return flag;
		} else if (event.getAction() == MotionEvent.ACTION_UP) {
			boolean flag = finishDrag(event);
			Log.e("PuzzleView", "finishDrag:" + flag);
			return flag;
		} else {
			return false;
		}
	...
	private void doMove(int dragDirection, int count) {
		if (puzzle.move(dragDirection, count)) {
			onMatchOrSolved(puzzle.isSolved());
		} else {
			onNoKnown();
		}
	...
	private void onMoved(int count) {
		activity.onMoved(count);
	}

	private void onNoKnown() {
		activity.onNoKnown();
	}
	
	private void onMatchOrSolved(boolean solved) {
		activity.onMatchOrSolved(solved);
	}
